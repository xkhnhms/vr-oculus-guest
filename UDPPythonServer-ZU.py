...
import sys
import math
sys.path.insert(0,r'E:\codes\SDK\lib64') #JAKA SDK path
import jkrc
from numpy import *
from utils import *
import threading
...
class Robot():                             ##创建机器人线程类
    def __init__(self):                                    ##初始化创建机器人并登录
        ...
        # self.robot = jkrc.RC('172.30.0.79')
    ...
    def run(self):                                         ##start时机器人回到设定的初始位置
        ...
        self.robot.servo_move_use_joint_LPF(0.2)
        # self.robot.set_user_frame_id(0)
        time.sleep(0.1)


        joint_pos = [0.04398210451818206, 1.5808151070257108, 1.774332306298577, 1.3353411372029384, -1.51052762936927, 0.839921590541316] #zu7
...
while not done:
    ...
    if lis_data:
    #格式化坐标信息
        pos_x = float(lis_data[0]) * 300
        pos_y = float(lis_data[1]) * 300
        pos_z = float(lis_data[2]) * 300
        ...
        # quaternions=[]
        # quaternions.append([-pos_x, pos_y, pos_z,quaternion])
    ...
    #执行一次机器人setting
        ...
        write_csv('./data/getSocket.csv', [num + 1, pos_x, pos_y, pos_z, quaternion, get_data_time1])
        # print(-pos_z, pos_x, -pos_y, quaternion)
        Rb.move(-pos_x, pos_y, pos_z,quaternion)
        get_data_time2 = time.time()
        write_csv('./data/movingOver.csv',
              [num + 1, pos_x, pos_y, pos_z, quaternion, get_data_time2, get_data_time2 - get_data_time1])
